...
{
    /// <summary>
    /// Draws moving water across a batch of tiles.
    /// <para>
    ...
    {
        private const uint Seed = 0x2B7F91C5u;
        ...
        private const int DepthLattice = 9;
        private const float DepthStrength = 44f;

        /// <summary>How much the wave gradient tips the surface normal.</summary>
        private const float Steepness = 0.055f;

        private const float Shine = 55f;
        private const float GlintStrength = 0.8f;

        private static readonly SKColor WaterDeep = new(0x1B, 0x45, 0x66);
        private static readonly SKColor WaterMid = new(0x2F, 0x76, 0x9C);
        private static readonly SKColor WaterShallow = new(0x51, 0xA6, 0xBC);
        private static readonly SKColor Foam = new(0xDC, 0xEF, 0xF3);
        ...
        /// <summary>
        /// Longest wave the texture carries, as a fraction of the block. Nothing spans the
        ...
        private static readonly ConcurrentDictionary<int, Lazy<WaveSurface>> Surfaces = new();
        ...
        private static readonly ConcurrentDictionary<int, SKPaint> DepthPaints = new();
        private static readonly Lazy<SKImage> DepthField = new(BuildDepthField, isThreadSafe: true);
        ...
        /// <summary>
        /// Draws every water tile in the batch: the current phase, the next one faded in over
        /// it, and the map-space depth field on top.
        /// </summary>
        ...
        {
            foreach (var key in Surfaces.Keys)
            {
                if (Surfaces.TryRemove(key, out var surface) && surface.IsValueCreated)
                {
            ...
            foreach (var key in DepthPaints.Keys)
            {
                if (DepthPaints.TryRemove(key, out var paint))
                {
        ...
        private static WaveSurface GetSurface(int tileSize)
        {
            if (Surfaces.TryGetValue(tileSize, out var cached) && cached.IsValueCreated)
            {
                cached.Value.LastUsed = Environment.TickCount64;
                return cached.Value;
            }

            var lazy = Surfaces.GetOrAdd(tileSize, static size => new Lazy<WaveSurface>(() =>
            {
        ...
        private static void Trim(int keep)
        {
            ...
            foreach (var size in candidates)
            {
                ...
                if (Surfaces.TryRemove(size, out var victim) && victim.IsValueCreated)
                {
        ...
        private static WaveSurface BuildSurface(int tileSize)
        {
        ...
        private static float[] TurbidityFor(int edge) => Turbidities.GetOrAdd(edge, static blockEdge =>
        {
            ...
            for (var py = 0; py < blockEdge; py++)
            {
                for (var px = 0; px < blockEdge; px++)
                    field[py * blockEdge + px] = PeriodicFbm(px * scale, py * scale, 3, 3, Seed ^ 0x7A1Du);
            }
        ...
        /// </para>
        /// </summary>
        private static SKImage BuildPhase(int edge, int phase, int phases)
        {
            ...
            for (var py = 0; py < edge; py++)
            {
                ...
                for (var px = 0; px < edge; px++)
                {
                    ...
                    var normal = Normalize(-du * Steepness, -dv * Steepness, 1f);
                    ...
                    var body = crest < 0.5f
                        ? LerpColor(WaterDeep, WaterMid, crest * 2f)
                        : LerpColor(WaterMid, WaterShallow, (crest - 0.5f) * 2f);

                    body = Shade(body, (turbidity[py * edge + px] - 0.5f) * 0.14f);
                    ...
                    var color = LerpColor(body, Foam, Clamp01(glint + foam * 0.5f));
        ...
        /// shallower water.
        /// </summary>
        private static SKImage BuildDepthField()
        {
            ...
            for (var ty = 0; ty < DepthPeriod; ty++)
            {
                ...
                {
                    var n = PeriodicFbm(tx * scale, ty * scale, DepthLattice, 4, Seed ^ 0x3C90u);
                    var level = 128f + (n - 0.5f) * 2f * DepthStrength;
        ...
        private static SKPaint GetDepthPaint(int tileSize) =>
            DepthPaints.GetOrAdd(tileSize, static size => new SKPaint
            {
